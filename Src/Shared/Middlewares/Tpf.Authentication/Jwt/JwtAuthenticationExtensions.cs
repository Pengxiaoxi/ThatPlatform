        ...
        public static void AddJwtBearerAuthentication(this IServiceCollection services)
        {
            ...
            .AddJwtBearer(JwtBearerDefaults.AuthenticationScheme, options =>
            {
                options.TokenValidationParameters = new TokenValidationParameters
                {
                    ...
                    ValidateIssuer = false, // 是否验证签发者。默认为true。注意，如果设置了TokenValidationParameters.IssuerValidator，则该参数无论是何值，都会执行。

                    ValidAudience = jwtOptions.Audience, // 有效的受众，即验证Jwt的Payload部分的aud。默认为null。
                    ValidateAudience = false, // 是否验证受众。默认为true。注意，如果设置了TokenValidationParameters.AudienceValidator，则该参数无论是何值，都会执行。

                    IssuerSigningKey = jwtOptions.SymmetricSecurityKey, // 用于验证Jwt签名的密钥。对于对称加密来说，加签和验签都是使用的同一个密钥；对于非对称加密来说，使用私钥加签，然后使用公钥验签。
                    ValidateIssuerSigningKey = false, // 是否使用验证密钥验证签名。默认为false。注意，如果设置了TokenValidationParameters.IssuerSigningKeyValidator，则该参数无论是何值，都会执行。
